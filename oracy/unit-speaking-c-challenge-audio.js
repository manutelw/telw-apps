...
function install(){const reps=[...document.querySelectorAll('.c-rep[data-rep="3"]')];if(reps.length<3)return false;reps.forEach(rep=>{if(rep.querySelector('.c-hear-challenge'))return;const model=rep.querySelector('.model');const line=model?.querySelector('div');if(!model||!line)return;const controls=document.createElement('div');controls.style.marginTop='8px';controls.innerHTML='<button type="button" class="c-hear-challenge">🔊 Hear coach challenge</button> <span class="c-challenge-status" aria-live="polite"></span>';
model.appendChild(controls);const btn=controls.querySelector('.c-hear-challenge'),status=controls.querySelector('.c-challenge-status');btn.addEventListener('click',()=>play(btn,line.textContent.trim(),status));});return true}
