...
export async function bridgeAdminHtml(request,response){
  ...
  if(ok){
    for(const [slug,target] of Object.entries(TARGETS))html=html.replaceAll(target,'/__admin/'+slug);
  }
  const script=ok
    ? `<script id="clarionAdminBrowserBridge">(function(){try{localStorage.setItem('clarion_device_v1','ADMIN');['PI_PRACTICE','GD_PRACTICE','PCL','WCT','LIVE_MOCK'].forEach(function(p){localStorage.setItem('clarion_entitlement_'+p,'ADMIN')})}catch(e){}})();</script>`
    : '';
  if(script)html=html.replace(/<head([^>]*)>/i,'<head$1>'+script);
